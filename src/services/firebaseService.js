...
class Firebase {
  ...
  signInWithGithub = () =>
    this.auth.signInWithPopup(new this.app.auth.GithubAuthProvider());

  signOut = () => this.auth.signOut();

  passwordReset = (email) => this.auth.sendPasswordResetEmail(email);

  addUser = (id, user) => this.db.collection('userData').doc(id).set(user);

  getUser = (id) => this.db.collection('userData').doc(id).get();
  ...
  reauthenticate = (currentPassword) => {
    const user = this.auth.currentUser;
    const cred = this.app.auth.EmailAuthProvider.credential(
      user.email,
      currentPassword
    );
